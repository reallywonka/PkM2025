...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderContent(lessonId) {
    try {
      // Hanya ambil dari data JavaScript
      const lesson = tutorials.javascript.lessons.find((l) => l.id === lessonId);
      if (lesson) {
  ...
  function renderSidebar() {
    // Hanya render sidebar JavaScript
    const topic = tutorials.javascript;
    if (!topic) {
  ...
  function generateNavMenus() {
    ...
    tutorialTopics.forEach((topicKey) => {
      ...
      let linksJS = `<h2>${topicData.title}</h2>`;
      linksJS += `
        <a 
          href="${pageUrl}" 
    ...
    Object.keys(References).forEach((key) => {
      if (Object.prototype.hasOwnProperty.call(References, key)) {
        ...
        if (typeof category === "object" && category.title) {
          const itemDiv = document.createElement("div");
          itemDiv.className = "nested-navigation-item";
          let linksJS = `<h2>${category.title} References</h2>`;
    ...
    Object.keys(Excersices).forEach((key) => {
      if (Object.prototype.hasOwnProperty.call(Excersices, key)) {
        ...
        if (typeof category === "object" && category.title) {
          const itemDiv = document.createElement("div");
          itemDiv.className = "nested-navigation-item";
          let linksJS = `<h2>${category.title} Excercise</h2>`;
          if (Array.isArray(category.lessons)) {
             linksJS += `
              <a href="#" class="nav-topic-link" data-topic="${key}">
  ...
  // Langsung muat sidebar dan konten JavaScript
  renderSidebar();
  renderContent("js-intro"); // Langsung tampilkan materi pertama
});